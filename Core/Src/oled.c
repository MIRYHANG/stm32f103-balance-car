...
void OLED_GPIO_Init(void)
{
	...
	/* PB8/PB9: software I2C, open-drain output. */
...
void OLED_ShowString(int16_t X, int16_t Y, const char *String, uint8_t FontSize)
{
	...
	{
		uint8_t CharLength;
		uint8_t FirstByte = (uint8_t)String[i];

		if ((FirstByte & 0x80U) == 0x00U) { CharLength = 1; }
		else if ((FirstByte & 0xE0U) == 0xC0U) { CharLength = 2; }
		else if ((FirstByte & 0xF0U) == 0xE0U) { CharLength = 3; }
		else if ((FirstByte & 0xF8U) == 0xF0U) { CharLength = 4; }
		else { CharLength = 1; }

		uint8_t copied = 0;
		while (copied < CharLength && String[i] != '\0')
		{
			SingleChar[copied++] = String[i++];
		}
		SingleChar[copied] = '\0';
